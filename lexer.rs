// This file is used to implement the lexer
// Look at https://github.com/OUCompilers/cs4100-sp20/blob/master/in-class/lexer-parser/src/lexer.rs
// for an example lexer

use regex::Regex;

// If you follow the example linked above, you may wind up with a `lex` function that
// hits over 100 lines of code. That is okay for this specific function, we will not
// take points off for this particular function.
...
fn lex<'a>(l: &mut LexerState<'a>) -> Result<Tok, String> {
    ...
    //LT
    else if s.starts_with("<") { lex_upd!(l, 1, Tok::LT) }
    //OPENPAREN
    else if s.starts_with("(") { lex_upd!(l, 1, Tok::OPENPAREN) }
    //CLOSEPAREN
    else if s.starts_with(")") { lex_upd!(l, 1, Tok::CLOSEPAREN) }
    //EQ
    else if s.starts_with("==") {l.rest = s.split_at(1).1; lex_upd!(l, 1, Tok::EQ) }
    //negative
                ...
                match Regex::new(r"^\A[[:digit:]]+").unwrap().find(s) {
                    ...
                    None => {
                        if s.len() > 0 {
                            ...
                            Err(format!("Program ended unexpectedly"))
                        }
                    }
                }
            }
